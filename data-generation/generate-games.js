...
async function connectAndGenerateGames() {
  ...
  try {
    ...
    async function generateGame() {
      ...
      const superCardsCollection = db.collection('superheroes_cards');
      const [user1, user2] = await iniUsersCollection.aggregate([{ $sample: { size: 2 } }]).toArray();
      const pipeline = [
        { $sample: { size: 6 } },
        {
          $project: {
      ...
      const cards = await superCardsCollection.aggregate(pipeline).toArray();      
    ...
    async function generateGames(numberOfGames) {
      ...
      gamesCollection.insertMany(gamesArray);
      console.log('Done!');
    }
    
    await generateGames(DEFAULT_NUMBER_OF_GAMES);
